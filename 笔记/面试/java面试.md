## 一、网络
  ...
  ### 3.Https的工作流程
      1.完成TCP三次同步握手，建立连接
      2.客户端验证服务端的数字证书，如果通过，进入3
      3.DH算法协商对称加密的算法及秘钥，hash算法的秘钥
      4.ssl安全加密隧道协商完成，将协商的对称加密算法及秘钥，hash算法的秘钥都是以非对称(RSA)的方式传输的
      5.以对称加密的方式进行数据传输，用协商的对称加密的秘钥及算法对数据加密，保证数据的机密性；用协商的hash算法，进行数据完整性保护，保证数据不被篡改
  ### 4.Http请求方法
## 二 、java
  ### 1.线程与进程
  ### 2.死锁是什么？死锁产生的条件
...
### 10. java内存模型
      堆（heap）：堆内存，线程共享，所有的对象实例以及数组都要在堆上分配。回收器主要管理的对象。
      方法栈（JVM Stack）：栈内存，线程私有、存储局部变量表、操作栈、动态链接、方法出口，对象指针。
      方法区（MEATHOD AREA）：线程共享，存储类信息、常量、静态变量、即时编译器编译后的代码。     
      本地方法栈（NATIVE METHOD STACK）：线程私有。为虚拟机使用到的Native 方法服务。如Java使用c或者c++编写的接口服务时，代码在此区运行。
      PC寄存器（PC Register 程序计数器）：线程私有。指向下一条要执行的指令。
...
### 11. volatile关键字
      ...
      防止重排序：通过插入内存屏障。
      dcl(double check locking)双重检查锁失效
      原因：
      Help help = new Help()这个操作可以拆成：
      1、栈内存开辟空间给help引用
      2、堆内存开辟空间准备初始化对象
      3、调用对应的构造函数做对象的初始化操作
      4、栈中引用指向这个堆内存空间地址
      多线程下，可能指令重排，1，2，4，3，这样到4之后，help已经指向了一个堆内存null,help=null，
